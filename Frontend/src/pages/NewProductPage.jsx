...
const NewProductPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (isNaN(price)||Number(price)<0) return setError('El precio debe ser un número positivo.');
    if (isNaN(stock)||Number(stock)<1) return setError('El stock debe ser al menos 1.');
    try {
      setLoading(true);
      await API.post('/products', {
        title, description, price: Number(price), category, condition,
        stock: Number(stock), images,
      });
        ...
        <div className="card flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            <input name="price" value={form.price} onChange={handleChange}
              placeholder="Precio COP *" type="number" min="0" className="input-base flex-1"/>
            <input name="stock" value={form.stock} onChange={handleChange}
              placeholder="Cantidad *" type="number" min="1" className="input-base w-28"/>
          </div>
