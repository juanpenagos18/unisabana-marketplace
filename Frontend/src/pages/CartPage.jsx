  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await API.post('/orders', {
        cartItems:  items.map(i => ({ productId: i._id, qty: i.qty })),
        totalPrice: total,
      });
      ...
      <Layout user={user} onLogout={handleLogout}>
        <div className="max-w-md mx-auto py-16 flex flex-col items-center gap-5 text-center">
          <div className="w-20 h-20 rounded-full flex items-center justify-center text-4xl"
            style={{ backgroundColor: 'var(--color-surface-alt)' }}>🎉</div>
          <h2 className="text-2xl font-bold" style={{ color: 'var(--color-primary)',
            fontFamily: 'Playfair Display, serif' }}>
            ¡Orden confirmada!
          </h2>
          <p className="text-gray-500 text-sm leading-relaxed">
            Tu orden fue generada exitosamente. Los vendedores se contactarán contigo pronto.
          </p>
          {orderId && (
            ...
            <div className="flex flex-col gap-3">
              {items.map(item => (
                <div key={item._id} className="card flex gap-4 items-center">
                  ...
                  {/* Info — título también es clickeable */}
                  <div className="flex-1 min-w-0">
                    <button
                      onClick={() => navigate(`/products/${item._id}`)}
                      className="font-semibold text-sm truncate text-left w-full hover:underline"
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <button
                        ...
                        className="w-7 h-7 rounded-lg flex items-center justify-center text-sm font-bold
                          border transition-colors disabled:opacity-30 hover:bg-gray-100"
                      ...
                      <span className="text-xs text-gray-400">de {item.stock}</span>
                    </div>
                  </div>
            ...
            <div className="card mt-2">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm text-gray-500">
                  Productos ({items.reduce((s, i) => s + i.qty, 0)} unidades)
                </span>
